...
const ColorSelect: React.FC<ColorSelectProps> = ({
	...
	const selectedColor = useMemo(
		() => tailwindColors.find((c) => c.value === value),
		[value]
	);
	...
	return (
		<div className=''>
			<Listbox value={value} onChange={onChangeValue}>
				<ListboxButton
					className={
						'border rounded-sm p-2 w-full text-left capitalize flex items-center'
					}
				>
					<ColorBadge bgClass={selectedColor?.value ?? ''} className='w-45' />
				</ListboxButton>
				<ListboxOptions
					anchor='bottom start'
					className='h-50 my-1 p-2 border rounded-sm bg-neutral-800 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1'
				>
