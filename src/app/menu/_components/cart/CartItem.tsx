...
const CartItem: React.FC<CartItemProps> = ({ cartItem, itemNo, onEdit, onDelete }) => {
	...
	return (
		...
		>
			...
			<div
				className={clsx(
					'relative px-3 py-3 flex items-center justify-between transition-transform duration-300 ease-in-out'
				)}
			>
				<div className='flex items-center gap-2'>
					<span className='rounded-sm w-3 h-3 pt-[2px] flex items-center justify-center text-[8px] text-gray-400 shrink-0'>
						{itemNo}
					</span>
			...
			>
				<button 
					type='button'
					onClick={onEdit}
					className='flex-1 flex items-center justify-center text-sm font-medium cursor-pointer gap-2 hover:bg-neutral-700'>
					<Pencil size={14} />
					<span>Edit</span>
				</button>
				<button 
